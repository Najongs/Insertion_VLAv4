#!/usr/bin/env python
"""
Training script for ACT (Action Chunking Transformer) on New HDF5 VLA Dataset

This script trains ACT policy for precise needle insertion task.
ACT is proven effective for insertion tasks (ALOHA peg insertion).

Usage:
    python train_act.py --config train_config_act.yaml

DDP Usage:
    torchrun --nproc_per_node=5 train_act.py --config train_config_act.yaml
"""
...
def create_dataset_from_config(config: Dict) -> torch.utils.data.Dataset:
    ...
    # Get n_obs_steps from policy config (ACT uses 1)
    n_obs_steps = policy_cfg.get("n_obs_steps", 1)
    logger.info(f"Using n_obs_steps={n_obs_steps} for ACT (will be squeezed)")

    # Create dataset with squeeze enabled for ACT compatibility
    dataset = create_hdf5_lerobot_dataset(
        ...
        # Squeeze temporal dimension for ACT (n_obs_steps=1 -> no temporal dimension)
...
def create_policy_from_config(config: Dict, device: torch.device) -> ACTPolicy:
    """Create and configure ACT policy."""
    from lerobot.configs.types import FeatureType, PolicyFeature
    from dataclasses import replace

    policy_cfg = config["policy"]

    # Load pretrained ACT model (insertion-specialized)
    pretrained_model_id = policy_cfg.get(
        "pretrained_model_id",
        "lerobot/act_aloha_sim_insertion_human"
    )

    logger.info(f"Loading ACT policy from: {pretrained_model_id}")
    pretrained_policy = ACTPolicy.from_pretrained(pretrained_model_id)

    # Create new config with our camera names
    logger.info("Creating new config with our dataset's camera names...")
    new_config = replace(
        pretrained_policy.config,
        n_obs_steps=policy_cfg.get("n_obs_steps", 1),
        chunk_size=policy_cfg.get("chunk_size", 100),
        n_action_steps=policy_cfg.get("n_action_steps", 100),
        temporal_ensemble_coeff=policy_cfg.get("temporal_ensemble_coeff", None),
        optimizer_lr=config["optimizer"].get("lr", 1e-5),
        optimizer_lr_backbone=policy_cfg.get("lr_backbone", 1e-5),
        input_features={
    ...
    logger.info(f"  temporal_ensemble_coeff: {policy.config.temporal_ensemble_coeff}")
    logger.info(f"  lr: {policy.config.optimizer_lr}")
    logger.info(f"  lr_backbone: {policy.config.optimizer_lr_backbone}")
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        # Forward pass
        # ACT policy returns (loss_tensor, loss_dict)
...
def main():
    parser = argparse.ArgumentParser(description="Train ACT on HDF5 VLA Dataset")
    ...
    try:
        ...
        # Create optimizer (ACT has custom parameter groups for backbone)
