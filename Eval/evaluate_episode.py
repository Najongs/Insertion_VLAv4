#!/usr/bin/env python
"""
Evaluate a trained SmolVLA model on a single episode, frame by frame.

This script evaluates each frame in an episode and compares predicted actions
with ground truth actions, providing detailed per-frame analysis.

Usage:
PYTHONPATH=/home/najo/NAS/VLA/Insertion_VLAv4/lerobot/src:$PYTHONPATH python3 /home/najo/NAS/VLA/Insertion_VLAv4/Eval/evaluate_episode.py \
    --checkpoint /home/najo/NAS/VLA/Insertion_VLAv4/Train/outputs/train/smolvla_needle_insertion/checkpoints/checkpoint_step_2000.pt \
    ...
    --output_dir /home/najo/NAS/VLA/Insertion_VLAv4/Eval/outputs/episode_eval_ep2 \
...
def prepare_observation(episode_data, frame_idx, preprocessor, device, policy, task_instruction="Insert needle into target point"):
    ...
    # Prepare observation dict (similar to dataset format)
    observation = {
    ...
    # Add language tokens (tokenize task instruction)
    # Use the policy's tokenizer to properly tokenize the instruction
...
def predict_action(policy, observation, device):
    """Predict action for a single observation."""
    with torch.no_grad():
        # Forward pass (use select_action, not select_actions)
        ...
        if len(action.shape) == 2:  # [batch, action_dim]
            ...
            return action[:, 0, :].cpu().numpy()[0]  # Take first action
        else:
...
def evaluate_episode(
    ...
    for frame_idx in range(num_frames):
        ...
        try:
            ...
            # Predict action
            pred_action = predict_action(policy, observation, device)

            # Handle dimension mismatch (model may predict 6D, gt is 7D with gripper)
...
def plot_results(results, output_dir: Path):
    ...
    fig.suptitle('Prediction Errors Over Episode', fontsize=16)
    ...
    # Plot 2: Per-dimension comparison (first 100 frames for clarity)
    ...
    fig.suptitle(f'Ground Truth vs Predicted Actions (First {max_frames_to_plot} Frames)', fontsize=16)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate episode frame by frame")
    ...
    # Note: Preprocessor creation may need adjustment based on actual checkpoint structure
    preprocessor = None  # For now, we'll handle preprocessing manually
